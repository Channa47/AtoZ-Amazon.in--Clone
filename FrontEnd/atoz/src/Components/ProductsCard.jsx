...
const ProductsCard = ({img, title,price, id}) => {
  ...
  return (
    <div>
      <Box border='1px' borderColor='gray.300' display="flex" align='left' p='4' bg='white' cursor='pointer'>
      
      <Box w='20%' mr={5} ml={5}>
      <Link to={`/products/${id}`}> <Image src={img}  m='auto'></Image></Link>
      </Box>
      <Link to={`/products/${id}`}>
      <Box >
        <Text  mt='2'>{title}</Text>
        <Box  display="flex" fontSize='sm' mt='2' m='auto'>  
        <Text fontWeight='medium' fontSize="2xl">₹{price}</Text>
        <Text  ml="2" color='grey' fontWeight='medium' mt={2}> ₹<del>5000</del> </Text>
        <Text  ml="2" color='green' mt={2}>₹4200</Text>    
         </Box>
